...
class DashBoard(ctk.CTkFrame):
    ...
    def __init__(self, master=None, **kwargs):
        ...
        self.left_side_panel = ctk.CTkFrame(
            self.main_container, width=200, corner_radius=10)
        ...
        self.logo_label = ctk.CTkLabel(
            self.left_side_panel, text="MTA Website Tools \n", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=1, column=0, padx=20, pady=(20, 10))

        # logo MTA
        current_path = os.path.dirname(os.path.realpath(__file__))
        self.img = ctk.CTkImage(Image.open(
            current_path + "/assets/image/mta.png"), size=(70, 70))
        self.logo_label_img = ctk.CTkLabel(
            self.left_side_panel, image=self.img, text="")
        self.logo_label_img.grid(row=0, column=0, padx=20, pady=(30, 10))

        # button to select correct frame IN self.left_side_panel WIDGET
        self.bt_dashboard = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/home.png"), size=(30, 30)), text="    Dashboard", command=self.dashboard)
        self.bt_dashboard.grid(row=2, column=0, padx=20, pady=10)

        self.bt_scan = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/search.png"), size=(30, 30)), text="    Scan", command=self.scan)
        self.bt_scan.grid(row=3, column=0, padx=20, pady=10)

        self.bt_exploit = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/exploit.png"), size=(30, 30)), text="    Exploit", command=self.exploit)
        self.bt_exploit.grid(row=4, column=0, padx=20, pady=10)

        self.bt_history = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/history.png"), size=(30, 30)), text="    History", command=self.history)
        self.bt_history.grid(row=5, column=0, padx=20, pady=10)

        self.bt_help = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/help.png"), size=(30, 30)), text="    Help", command=self.help)
        self.bt_help.grid(row=6, column=0, padx=20, pady=10)

        self.bt_setting = ctk.CTkButton(self.left_side_panel, width=180, height=35, anchor="w", image=ctk.CTkImage(
            Image.open(current_path + "/assets/icons/setting.png"), size=(30, 30)), text="    Settings", command=self.setting)
        ...
        self.right_side_panel = ctk.CTkFrame(
            self.main_container, corner_radius=10, fg_color="black", bg_color='black')
    ...
    def exploit(self):
        self.clear_frame()
        self.main_dashboard = Exploit(self.right_dashboard)
        self.main_dashboard.pack(
            side=tkinter.LEFT, fill=tkinter.BOTH, expand=True, padx=0, pady=0)
    ...
    def help(self):
        ...
        self.bt_from_frame4 = ctk.CTkButton(
            self.right_dashboard, text="help", command=lambda: print("test cats"))
        self.bt_from_frame4.grid(row=0, column=0, padx=20, pady=(10, 0))

    def history(self):
        self.clear_frame()
        self.main_dashboard = History(self.right_dashboard)
        self.main_dashboard.pack(
            side=tkinter.LEFT, fill=tkinter.BOTH, expand=True, padx=0, pady=0)
    ...
    def setting(self):
        self.clear_frame()
        self.bt_from_frame4 = ctk.CTkButton(
            self.right_dashboard, text="settings", command=lambda: print("test cats"))
        self.bt_from_frame4.grid(row=0, column=0, padx=20, pady=(10, 0))
